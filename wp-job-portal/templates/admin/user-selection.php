...
if (isset($wpjobportal_label)) {
	$wpjobportal_args['label'] = $wpjobportal_label;
}
if (isset($wpjobportal_show_username_on_selection)) {
	$wpjobportal_args['show_username_on_selection'] = $wpjobportal_show_username_on_selection;
}
if (isset($wpjobportal_selectors)) {
	$wpjobportal_args['selectors'] = $wpjobportal_selectors;
}

// listfor 1 shows all users. list for 2 shows only employers
$wpjobportal_listfor = 1;
if(isset(wpjobportal::$wpjobportal_data['admin_form_company']) && wpjobportal::$wpjobportal_data['admin_form_company'] == 1 ){
    $wpjobportal_listfor = 2;
}
?>

<?php if($wpjobportal_args['show_username_on_selection']): ?>
<span id="username-div"></span>
<?php endif; ?>
<a href="#" id="userpopup"><?php echo esc_html($wpjobportal_args['label']); ?></a>
        ...
        <div id="userpopupsearch">
            <div class="popup-form-fields-wrp">
                ...
                <div class="popup-form-btn-wrp">
                    <input type="submit" class="popup-search-btn" onclick="getUserListAfterSearch()" value="<?php echo esc_attr(__('Search', 'wp-job-portal'));?>" />
                    <input type="submit" class="popup-reset-btn" onclick="document.getElementById('name').value = '';document.getElementById('uname').value = ''; document.getElementById('email').value = '';getUserListAfterSearch()" value="<?php echo esc_attr(__('Reset', 'wp-job-portal'));?>" />
                </div>
            </div>
        </div>
    </div>            
    ...
    $wpjobportal_inline_js_script = "
        function updateuserlist(pagenum) {
            var username = jQuery('input#uname').val();
                ...
                jQuery.post(ajaxurl, {action: 'wpjobportal_ajax', wpjobportalme: 'user', task: 'getuserlistajax',name: name, uname: username, email: emailaddress,userlimit: pagenum, listfor: ".esc_attr($wpjobportal_listfor).", '_wpnonce':'". esc_attr(wp_create_nonce("get-user-list-ajax"))."' }, function (data) {
                if (data) {
        ...
        function getUserListAfterSearch() {
            ...
            var emailaddress = jQuery('input#email').val();
            var ajaxurl = \"". esc_url_raw(admin_url('admin-ajax.php')) ."\";
            jQuery.post(ajaxurl, {action: 'wpjobportal_ajax', wpjobportalme: 'user', task: 'getuserlistajax', name: name, uname: username, email: emailaddress,listfor: ".esc_attr($wpjobportal_listfor).", '_wpnonce':'". esc_attr(wp_create_nonce("get-user-list-ajax"))."'}, function (data) {
                if (data) {
                    jQuery('span#popup_title').html(jQuery('input#user-popup-title-text').val());
                    jQuery('div#popup-record-data').html(data);
                    setUserLink();
        ...
        function setUserLink() {
            jQuery('a.userpopup-link').each(function () {
                ...
                jQuery(anchor).click(function (e) {
                    ...
                    ";
                    if(isset($wpjobportal_args['selectors']['id'])){
                        $wpjobportal_inline_js_script .= "jQuery(\"". esc_js($wpjobportal_args['selectors']['id'])."\").val(id);";
                    }

                    if(isset($wpjobportal_args['selectors']['name'])){
                        $wpjobportal_inline_js_script .= "jQuery(\"". esc_js($wpjobportal_args['selectors']['name'])."\").val(name);";
                    }

                    if(isset($wpjobportal_args['selectors']['email'])){
                        $wpjobportal_inline_js_script .= "jQuery(\"". esc_js($wpjobportal_args['selectors']['email'])."\").val(email);";
                    }
                    if(in_array('credits', wpjobportal::$_active_addons)){
                        $wpjobportal_inline_js_script .= "jQuery('input.wpjobportal-form-save-btn').attr('credit_userid',id);";
                    }
                    $wpjobportal_inline_js_script .= "
