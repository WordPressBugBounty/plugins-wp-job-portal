...
class WPJOBPORTALCurrencyController {
    ...
    function handleRequest() {
        $wpjobportal_layout = WPJOBPORTALrequest::getLayout('wpjobportallt', null, 'currency');
        if (self::canaddfile($wpjobportal_layout)) {
            switch ($wpjobportal_layout) {
                ...
                case 'admin_formcurrency':
                    ...
                    WPJOBPORTALincluder::getJSModel('currency')->getCurrencybyId($wpjobportal_id);
                    break;
                default:
                    return;
            }
            $wpjobportal_module = (wpjobportal::$_common->wpjp_isadmin()) ? 'page' : 'wpjobportalme';
            $wpjobportal_module = WPJOBPORTALrequest::getVar($wpjobportal_module, null, 'currency');
    ...
    function savecurrency() {
        ...
        if (! wp_verify_nonce( $wpjobportal_nonce, 'wpjobportal_currency_nonce') ) {
             die( 'Security check Failed' );
        }
        if(!wpjobportal::$_common->wpjp_isadmin())
            return false;
        $wpjobportal_data = WPJOBPORTALrequest::get('post');
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('currency')->storeCurrency($wpjobportal_data);
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_currency&wpjobportallt=currency"));
        $wpjobportal_msg = WPJOBPORTALMessages::getMessage($wpjobportal_result, 'currency');
    ...
    function remove() {
        ...
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('currency')->deleteCurrencies($wpjobportal_ids);
        $wpjobportal_msg = WPJOBPORTALMessages::getMessage($wpjobportal_result, 'currency');
        WPJOBPORTALMessages::setLayoutMessage($wpjobportal_msg['message'], $wpjobportal_msg['status'],$this->_msgkey);
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_currency&wpjobportallt=currency"));
        wp_redirect($wpjobportal_url);
    ...
    function publish() {
        ...
        $wpjobportal_ids = WPJOBPORTALrequest::getVar('wpjobportal-cb');
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('currency')->publishUnpublish($wpjobportal_ids, 1); //  for publish
    ...
    function unpublish() {
        ...
        $wpjobportal_pagenum = WPJOBPORTALrequest::getVar('pagenum');
        $wpjobportal_ids = WPJOBPORTALrequest::getVar('wpjobportal-cb');
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('currency')->publishUnpublish($wpjobportal_ids, 0); //  for unpublish
        $wpjobportal_msg = WPJOBPORTALMessages::getMessage($wpjobportal_result, 'record');
        WPJOBPORTALMessages::setLayoutMessage($wpjobportal_msg['message'], $wpjobportal_msg['status'],$this->_msgkey);
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_currency&wpjobportallt=currency"));
        if ($wpjobportal_pagenum)
    ...
    function saveordering(){
        ...
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_currency"));
