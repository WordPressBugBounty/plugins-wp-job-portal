...
class WPJOBPORTALCareerlevelController {
    ...
    function handleRequest() {
        $wpjobportal_layout = WPJOBPORTALrequest::getLayout('wpjobportallt', null, 'careerlevels');
        if (self::canaddfile($wpjobportal_layout)) {
            switch ($wpjobportal_layout) {
                ...
                case 'admin_formcareerlevels':
                    ...
                    WPJOBPORTALincluder::getJSModel('careerlevel')->getJobCareerLevelbyId($wpjobportal_id);
                    break;
                default:
                    return;
            }
            $wpjobportal_module = (wpjobportal::$_common->wpjp_isadmin()) ? 'page' : 'wpjobportalme';
            $wpjobportal_module = WPJOBPORTALrequest::getVar($wpjobportal_module, null, 'careerlevel');
    ...
    function savecareerlevel() {
        ...
        if (! wp_verify_nonce( $wpjobportal_nonce, 'wpjobportal_careerlevel_nonce') ) {
             die( 'Security check Failed' );
        }
        if(!wpjobportal::$_common->wpjp_isadmin())
            return false;
        $wpjobportal_data = WPJOBPORTALrequest::get('post');
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('careerlevel')->storeCareerLevel($wpjobportal_data);

        $wpjobportal_msg = WPJOBPORTALMessages::getMessage($wpjobportal_result, 'careerlevel');
        WPJOBPORTALMessages::setLayoutMessage($wpjobportal_msg['message'], $wpjobportal_msg['status'],$this->_msgkey);
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_careerlevel&wpjobportallt=careerlevels"));
    ...
    function remove() {
        ...
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('careerlevel')->deleteCareerLevels($wpjobportal_ids);
        $wpjobportal_msg = WPJOBPORTALMessages::getMessage($wpjobportal_result, 'careerlevel');
        WPJOBPORTALMessages::setLayoutMessage($wpjobportal_msg['message'], $wpjobportal_msg['status'],$this->_msgkey);
    ...
    function publish() {
        ...
        $wpjobportal_ids = WPJOBPORTALrequest::getVar('wpjobportal-cb');
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('careerlevel')->publishUnpublish($wpjobportal_ids, 1); //  for publish
    ...
    function unpublish() {
        ...
        $wpjobportal_pagenum = WPJOBPORTALrequest::getVar('pagenum');
        $wpjobportal_ids = WPJOBPORTALrequest::getVar('wpjobportal-cb');
        $wpjobportal_result = WPJOBPORTALincluder::getJSModel('careerlevel')->publishUnpublish($wpjobportal_ids, 0); //  for unpublish
        $wpjobportal_msg = WPJOBPORTALMessages::getMessage($wpjobportal_result, 'record');
        WPJOBPORTALMessages::setLayoutMessage($wpjobportal_msg['message'], $wpjobportal_msg['status'],$this->_msgkey);
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_careerlevel&wpjobportallt=careerlevels"));
        if ($wpjobportal_pagenum)
    ...
    function saveordering(){
        ...
        $wpjobportal_url = esc_url_raw(admin_url("admin.php?page=wpjobportal_careerlevel"));
        wp_redirect($wpjobportal_url);
        exit;
    }
